...
public class DatePicker extends HBox {
	...
	DatePicker(CalendarPane calendarPane) {
		...
		//Get copy of date
		this.pickerDate = calendarPane.getPlanDate();
		...
		todayButton.setOnAction(e -> {
			...
			this.calendarPane.setViewMode(ViewMode.day);
			this.calendarPane.setPlanDate(this.pickerDate);
		});
		...
		for (int i = 0; i < 42; i++) {
			...
			if (i % 7 == 0) {
				...
				weekButton.setOnAction(e -> {
					this.pickerDate = followingDate;
					this.setVisible(false);
					this.calendarPane.setViewMode(ViewMode.week);
					this.calendarPane.setPlanDate(this.pickerDate);
				});
